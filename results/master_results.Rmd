...
```{r}
# MARCC
m_marcc <- read.table('marcc/bowtie-48.tsv', header=T, sep='\t')
m_marcc <- rbind(m_marcc, read.table('marcc/hisat-48.tsv', header=T, sep='\t'))
head(m_marcc)
```
...
```{r}
mtmp <- m_marcc %>% filter(tool == 'bowtie' & paired == 'unp')
ggplot(mtmp, aes(x=threads, y=seconds, color=lock, linetype=version)) +
  geom_line() + geom_point() + theme_bw() +
  labs(y="Normalized running time", x="# threads") +
...
```{r}
mtmp <- m_marcc %>% filter(tool == 'hisat' & paired == 'unp')
ggplot(mtmp, aes(x=threads, y=seconds, color=lock, linetype=version)) +
  geom_line() + geom_point() + theme_bw() +
  labs(y="Normalized running time", x="# threads") +
...
```{r}
m_marcc_4ival <- read.table('marcc/hisat-48-4th-interval.tsv', header=T, sep='\t')
mtmp <- m_marcc_4ival %>% filter(tool == 'hisat' & paired == 'pe')
ggplot(mtmp, aes(x=threads, y=seconds, color=lock, linetype=version)) +
  geom_line() + geom_point() + theme_bw() +
  labs(y="Normalized running time", x="# threads") +
  coord_cartesian(ylim=c(0, max(mtmp$seconds)*1.05))
