...
plot(xy$V1, jitter(xy$V2, factor=jitter_factor), ylim=c(0, 110),
...
xy <- read.table(paste(prefix, 'sensitive_no_io.tsv', sep=''))
plot(xy$V1, jitter(xy$V2, factor=jitter_factor),
	xlab='# simultaneous threads',
	ylab='Running time',
	main='Running time for each thread, no input or output locking')
dev.off()
...
for(sens in c('very-fast', 'fast', 'sensitive', 'very-sensitive')) {
	...
	plot(xy$V1, jitter(xy$V2, factor=jitter_factor), ylim=c(30, 200), col=rgb(0, 0, 0, 0.2),
		xlab='# simultaneous threads',
		ylab='Running time',
		main=sens)
}
