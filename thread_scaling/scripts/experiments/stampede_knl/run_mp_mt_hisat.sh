...
do
	...
	#perl -e '$N='${N}'; $rpt='${R}'; $T='${T}'; $tpp=$T/$N; $rpp=$rpt*$tpp; for $i (1..$N) {  $p=fork(); if($p==0) { $outf="hisat_mp_mt1_".$tpp."_".$rpp."_$i"; print "$tpp $rpp $i $T $N $rpt\n"; `build/hisat-batch32-tbb-q-tbbd-out32-p-s/hisat-align-s -p $tpp -u $rpp --sensitive -S /dev/null -x /dev/shm/hg19_hisat -U '${D}'/err.$i.mt -t --no-spliced-alignment --no-temp-splicesite --reads-per-out-batch 32 --reads-per-batch 32 --mm 2> hisat_mp_mt/$outf.err | sort > hisat_mp_mt/$outf`; exit(0); }} wait();'
	...
	#perl -e '$N='${N}'; $rpt='${R}'; $T='${T}'; $tpp=$T/$N; $rpp=$rpt*$tpp; for $i (1..$N) {  $p=fork(); if($p==0) { $outf="hisat_mp_mt2_".$tpp."_".$rpp."_$i"; print "$tpp $rpp $i $T $N $rpt\n"; `build/hisat-batch32-tbb-q-tbbd-out32-p/hisat-align-s -p $tpp -u $rpp --sensitive -S /dev/null -x /dev/shm/hg19_hisat -1 '${D}'/err.$i.mt -2 '${D}'/err.$i.mt2 -t --no-spliced-alignment --no-temp-splicesite --reads-per-out-batch 32 --reads-per-batch 32 --mm 2> hisat_mp_mt/$outf.err | sort > hisat_mp_mt/$outf`; exit(0); }} wait();'
