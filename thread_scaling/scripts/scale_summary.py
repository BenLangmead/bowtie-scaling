...
parser.add_argument('--input', nargs='+', help='Input files', required=True)
...
for fn in args.input:
    m = re.match("(.*)_([0-9]+)_([0-9]+)\.out", fn)
    fns = None
    if m is not None and m.group(3) == '0':
        nthreads = int(m.group(2))
        fns = [x for x in glob.glob(m.group(1) + '_' + m.group(2) + '_*.out')]
        assert len(fns) > 0
        print("Processing group '%s'" % str(fns), file=sys.stderr)
    elif m is not None:
        continue
    else:
        ...
        if m is None:
            raise RuntimeError("Could not parse filename '%s'" % fn)
        fns = [fn]
        ...
        with open(fn) as fh:
            for ln in fh:
                ...
                if 'node_changeovers:' in ln:
                    ...
                    node_changeovers[(thread_id, node_id)] = int(m2.group(2))
                elif 'cpu_changeovers' in ln:
                    m2 = re.match("thread: ([0-9]*) cpu_changeovers: ([0-9]*)", ln)
                    assert m2 is not None
                    thread_id = int(m2.group(1))
                    cpu_changeovers[(thread_id, node_id)] = int(m2.group(2))
                elif 'time:' in ln:
                    ...
                    thread_times[(thread_id, node_id)] = sc
                    if scatter_fh is not None:
                        scatter_fh.write('%d\t%0.03f\n' % (nthreads, sc))
        node_id += 1
    assert len(thread_times) == nthreads, (len(thread_times), nthreads)
