...
peak_throughput_table <- function(m) {
  mg <- m %>% group_by(series, pe, attempt, aligner, machine) %>%
    summarise(threads_div_max_threads=totthreads[which(threads_div_max == max(threads_div_max))[1]],
              threads_div_max=max(threads_div_max))
  tabular((Heading() * aligner * Heading() * series) ~ (Justify(c) * Heading() * identity * Heading() * machine * Heading() * pe * Justify(r) * (Heading('Threads') * threads_div_max_threads + Heading('Threads/sec') * Format(sprintf("%.2f")) * threads_div_max)), data=mg)
}
...
m <- rbind(m_st, m_bm)
m$aligner <- ifelse(m$aligner == 'bt', 'Bowtie', ifelse(m$aligner == 'bt2', 'Bowtie 2', 'HISAT'))
...
m$pe <- ifelse(m$pe == 'pe', '100 x 100 nt', '100 nt unpaired')
m$pe <- factor(m$pe)
m$machine <- factor(m$machine)

fn <- 'peak_throughput.tex_snippet'
